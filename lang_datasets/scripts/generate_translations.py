...
class GoogleTranslate:
    ...
    async def translate_all(self, texts, src_lang):
        total = len(texts)
        tasks = [
            self.translate_text(text, src_lang, idx, total)
...
class NLLBTranslate:
    ...
    def translate_text(self, text: str, src_lang: str, target_lang: str = "eng_Latn"):
        ...
        nllb_translation = self.tokenizer.batch_decode(
...
class LLMTranslate:
    ...
    def translate_text(self, text: str, src_lang: str, target_lang: str = "eng_Latn"):
        translate_prompt = (
            f"Translate the following text from {src_lang} to {target_lang}: {text}"
        )
...
if __name__ == "__main__":
    ...
    for flores_code, goog_code in LANGUAGES.items():
        ...
        progress_counter += 1
        logger.info(
            f"Processing translations for {flores_code}. Progress: {progress_counter}/{len(LANGUAGES)}"
        )

        # Get the flores sample file path
        FLORES_SAMPLE_FILE = f"./lang_datasets/flores200_dataset/sample_original/{flores_code}_sample.txt"
        ...
        # ##### Google Translate #####
        logger.info(
            f"Translating {len(translations)} translations from {flores_code} to English using Google Translate..."
        )
        goog_back_translations = asyncio.run(
            goog_translator.translate_all(translations, goog_code)
        )
        for english_sentence, translation, back_translation in zip(
            SELECTED_SENTENCES, translations, goog_back_translations
        ):
            translations_dict[english_sentence][flores_code] = {
                "original": translation,
                "goog_back_translation": back_translation,
            }

        ##### NLLB Translate #####
        logger.info(
            f"Translating {len(translations)} translations from {flores_code} to English using NLLB Translate..."
        )
        nllb_back_translations = nllb_translator.translate_all(
            translations, src_lang=flores_code
        )
        for english_sentence, translation, back_translation in zip(
            SELECTED_SENTENCES, translations, nllb_back_translations
        ):
            translations_dict[english_sentence][flores_code].update(
                {"nllb_back_translation": back_translation}
            )

        ##### DeepSeek or ChatGPT Translate #####
        logger.info(
            f"Translating {len(translations)} translations from {flores_code} to English using LLM Translate..."
        )
        llm_back_translations = llm_translator.translate_all(
            translations, src_lang=flores_code
        )
        for english_sentence, translation, back_translation in zip(
            SELECTED_SENTENCES, translations, llm_back_translations
        ):
            translations_dict[english_sentence][flores_code].update(
                {"llm_back_translation": back_translation}
            )

    # Save the translations to a JSON file
    OUTPUT_FILE = "./lang_datasets/flores200_dataset/flores_sample_translations.json"
